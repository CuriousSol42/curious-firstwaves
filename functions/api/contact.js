...
function allowedOrigin(request, env) {
  ...
  const fallbackOrigins = [
    "https://firstwaves.space",
    "https://www.firstwaves.space",
    "https://curious-firstwaves.pages.dev",
  ];
...
function cleanPage(value) {
  ...
  if (!page) return "firstwaves.space/contact";
...
function validatePayload(payload) {
  ...
  return {
    record: {
      ...
      source: "first_waves_contact".slice(0, LIMITS.source),
    },
  };
}
...
async function submitToSupabase(record, request, env) {
  ...
  const body = {
    p_target: env.CONTACT_TARGET || "curious",
  ...
  if (!response.ok) {
    return { error: "The note could not be received just now.", status: 502 };
  }
...
export async function onRequestOptions({ request, env }) {
  ...
  return new Response(null, {
    status: 204,
    headers: {
      "access-control-allow-origin": origin || "https://firstwaves.space",
